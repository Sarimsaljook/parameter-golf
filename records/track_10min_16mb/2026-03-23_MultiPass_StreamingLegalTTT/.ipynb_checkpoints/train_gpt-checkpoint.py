...
def main() -> None:
    ...
    # streaming legal score first TTT to score each batch before training on it
    ttt_passes = int(os.environ.get("TTT_PASSES", 3))
    ...
    if ttt_passes > 0:
        ...
        ttt_batch = 16
        chunk_stride = ttt_batch * seq_len
        ...
        log0(f"legal_ttt: {ttt_passes} passes, {rank_tokens} tokens/rank, lr={ttt_lr}")
        for pass_idx in range(ttt_passes):
            eval_model.load_state_dict(base_sd, strict=True)
            ...
            if pass_idx > 0:
                ...
                for name, p in eval_model.named_parameters():
                    ...
                    if ".mlp.proj" in name: proj_p.append(p)
                    elif ".mlp.fc" in name: fc_p.append(p)
                    else: other_p.append(p)
                ttt_opt = torch.optim.AdamW([
                    {"params": proj_p, "lr": ttt_lr * 3.0, "initial_lr": ttt_lr * 3.0},
                    {"params": fc_p, "lr": ttt_lr * 0.5, "initial_lr": ttt_lr * 0.5},
                    {"params": other_p, "lr": ttt_lr, "initial_lr": ttt_lr},
                ], weight_decay=0.0)
            ...
            for bi, bs in enumerate(positions):
                ...
                if vm.any():
                    best_nll[indices[vm]] = torch.minimum(best_nll[indices[vm]], nll[vm])
                if pass_idx > 0:
                    eval_model.train()
                    ...
                    ttt_opt.step()
                    eval_model.eval()
                if master_process and (bi + 1) % 100 == 0:
                    scored = (best_nll < float('inf')).sum().item()
                    avg = best_nll[best_nll < float('inf')].mean().item() if scored > 0 else 0
                    log0(f"  pass {pass_idx} [{bi+1}/{len(positions)}] scored={scored} avg_nll={avg:.4f}")
            if master_process:
                log0(f"  pass {pass_idx}: done")
        # Compute BPB from best_nll
        ...
        if distributed:
            ...
            dist.all_reduce(token_count, op=dist.ReduceOp.SUM)
            dist.all_reduce(byte_count, op=dist.ReduceOp.SUM)
        ttt_val_loss = (loss_sum / token_count).item()
        bpt = ttt_val_loss / math.log(2.0)
